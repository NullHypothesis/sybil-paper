...
CollecTor~\cite{collector}, a service run by The Tor Project to archive network
data such as consensuses, router descriptors, and network status votes.  Some of
the archived data dates back to 2004, allowing us to restore arbitrary Tor
network configurations in the last ten years.  Not all of CollecTor's archived
data, however, is relevant to our hunt for Sybils, which is why we only analyse
the following two:
...
\begin{tikzpicture}[node distance=0 cm,outer sep = 0pt,inner sep = 2pt]
	...
	\tikzset{field/.style={align=center,shape=rectangle,
		minimum height=4.5mm,minimum width=25mm,fill=gray!20}}

	\tikzset{blankfield/.style={align=center,shape=rectangle,
		minimum height=4.5mm,minimum width=25mm}}
