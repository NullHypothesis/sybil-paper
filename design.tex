\section{Design}
\label{sec:design}
We begin by presenting methods on detecting Sybil relays in the Tor network.
These methods are implemented in our tool sybilhunter, whose design philosophy
we will discuss as well.  Sybilhunter takes as input archived Tor network data
and then attempts to expose Sybil groups, as illustrated in
...
are similar in their \emph{behavior} (\S~\ref{sec:churn-time-series},
\S~\ref{sec:uptime-matrix}, and \S~\ref{sec:fingerprint-analysis}) and
\emph{appearance} (\S~\ref{sec:nearest-neighbor}).  In addition to our analysis
methods, the following sections discuss our datasets (\S~\ref{sec:datasets}) and
our threat model (\S~\ref{sec:threat_model}).
...
defenses.  This is reflected in our adversarial assumptions.  We assume our
adversaries to:
\begin{itemize}
	...
	\item Have some redundancy in their behavior or appearance, e.g., similar
		configuration.
\end{itemize}

Note that our methods fail if Sybils exhibit no redundancy.  In a way, our
methods exploit our adversaries' 
...
implementation, building a vp-tree for 6,235 relays takes approximately 10
