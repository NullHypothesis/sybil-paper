...
apply it to newly incoming data.  However, this puts us in an adversarial
...
\emph{does} run more than one Tor relay and exhibits redundancy in their relay
configuration, or uptime sequence.  An adversary further \emph{can} know how
\sys's modules work, run active or passive attacks, and make a limited effort to
stay under the radar, by diversifying parts of their configuration.  To detect
Sybils, however, our heuristics require \emph{some} redundancy.
...
parameters such as port numbers and nicknames cause similar appearance and
Sybils behave similarly when they reboot simultaneously, or exhibit the same
...
we adapt Godfrey et al.'s~\cite{Godfrey2006a} formula, which yields a churn
value that captures both systems that joined and systems that left the network.
...
as well as for relays that left ($\sigma_{g}$).  $C_{t}$ is the network
consensus at time $t$, and $\setminus$ denotes the complement between two
consensuses, i.e., the relays that are in the left operand, but not the right
operand.  We define $\sigma_{n}$ and $\sigma_{g}$ as
...
together relays whose uptime is highly correlated.  The churn technique is
...
(HSDirs).  As of February 2016, 46\% of all Tor relays serve as HSDir.  A
...
turn string $s_{1}$ into $s_{2}$.  Our algorithm extracts several configuration
parameters of two relays, concatenates them into two strings, and determines
...
port.  To turn string $s_2$ into $s_1$, six operations are necessary; three
...
from what Sommer and Paxson call the semantic gap~\cite[\S III.C]{Sommer2010a},
a gap in understanding what a system's output means and how it should be acted
upon.  Our techniques automate much of the \emph{detection} of Sybils, but
\emph{blocking} them involves human interaction.  If The Tor Project must spend
hours to understand and verify \sys alerts, our techniques are ultimately
useless.  Therefore, we took special care to narrow the semantic gap by avoiding
the use of opaque classification methods because their output---frequently
distances in high-dimensional vector space---is difficult to interpret.
Finally, in accordance with the Unix tool design philosophy~\cite{Pike1983a},
\sys produces CSV-formatted text output that is easy to pipe into command line
tools such as cut, grep, and awk for further analysis.
